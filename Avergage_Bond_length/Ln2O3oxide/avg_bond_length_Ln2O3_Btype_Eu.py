...
####################################################################
#
# Calculation of the average bond legth in the Eu2O3-Btype
#
####################################################################
...
On = read_atom_positions(file, 9)  # read L atomic positions
Ln = read_atom_positions(file, 6) # read O atomic positions
Cn = repeat_cells(2)               # shifting cell from -2, ..., 2 in x, y, z.

(LOd, LOn) = average_distance(Ln, On, Cn, mlat, 4.312)
(LLd, LLn) = average_distance(Ln, Ln, Cn, mlat, 6.268)
