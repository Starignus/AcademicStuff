...
####################################################################
#
# Calculation of the average bond legth in the ThMo2O3
#
####################################################################
...
Thn = read_atom_positions(file, 4)  # read Th atomic positions
Mon = read_atom_positions(file, 8)  # read Mo atomic positions
On = read_atom_positions(file, 32) # read O atomic positions
Cn = repeat_cells(1)               # shifting cell from -2, ..., 2 in x, y, z.

(ThOd, TOn) = average_distance(Thn, On, Cn, mlat, 2.5)
(MoOd, MOn) = average_distance(Mon, On, Cn, mlat, 3.0)
(MoThd, MTn) = average_distance(Mon, Thn, Cn, mlat, 3.9)
